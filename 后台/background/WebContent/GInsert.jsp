...
<%@page import="java.sql.*" %><!调用sql包!>
 ...
 <jsp:useBean id="add" class="background.com.Bean.GInsertBean"scope="request"/><!使用javabean文件InsertBean并命名为add!>
 <jsp:useBean id="look" class="background.com.Bean.GqureyBean"scope="request"/><!使用javabean文件QueryBean并命为look!>
...
<body><font size=2> 
sid是主键，不可重复，请输入新记录的相应字段的值：
<form action="" method=post><!表单用于提交数据!>
输入商品id；<Input type="text" name="gid"size=10>
输入商品名称：<Input type="text" name="gname"size=10>
输入商品价格：<Input type="text" name="gprice"size=10>
